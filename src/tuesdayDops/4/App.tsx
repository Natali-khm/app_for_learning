...
function App() {
    ...
    function removeTask(id: string, todolistId: number) {
        setTodo(
            todo.map(
                (tl, i) => i === todolistId 
                ? {...tl, tasks: tl.tasks.filter(t => id !== t.taskId)}
                : tl
        ))}

    function addTask(title: string, todolistId: number) {
        setTodo(
            todo.map((tl, i) => i === todolistId 
    ...
    function changeStatus(id: string, isDone: boolean, todolistId: number) {
        setTodo(
            todo.map(
                (tl, i) => i ===  todolistId 
                ? {...tl, tasks: tl.tasks.map(t => id === t.taskId ? {...t, isDone} : t)}
                : tl
        ))}
    ...
    function changeFilter(value: FilterValuesType, todolistId: number) {
        setTodo(
            todo.map(
                (tl, i) => i === todolistId ? {...tl, filter: value} : tl
        ))}

    function removeTodolist(id: number) {
        setTodo(
            todo.filter((tl, i) => i !== id)
        )}
        ...
        <div className="App">
            {
                todo.map((tl,i) => {
                    ...
                    let tasksForTodolist: TasksType[] = allTodolistTasks;
